...
function themvaogiohang(x) {
  // 1. Tìm thẻ card bao quanh sản phẩm
  var card = x.closest(".card");

  // 2. Lấy dữ liệu sản phẩm
  var hinh = card.querySelector("img").src;
  var tensp = card.querySelector(".card-text").innerText;

  var giaText = card.querySelector("h5").innerText;
  var gia = parseInt(giaText.replace(/\D/g, ""));

  // 3. Lấy số lượng (tìm input gần button)
  var soluong = parseInt(
    x.parentElement.querySelector('input[name="soluong"]').value,
  );
